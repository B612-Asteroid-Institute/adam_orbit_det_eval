...
def run_looo_pipeline(
    ...
    orbit_fitter : OrbitFitter, optional
        OD backend for the differential-correction step.  Must implement
        ``adam_core.orbit_determination.OrbitFitter`` and be picklable.
        If ``None`` (default), falls back to ``fit_least_squares``.
